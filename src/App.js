...
class App extends Component {
  ...
  toggleDarkMode = () => {
    ...
    localStorage.setItem('darkmode', JSON.stringify({dark: this.state.dark ? false : true}));
  }
  ...
  componentDidMount = () => {
    ...
    fetch('http://urlreq.appspot.com/req?method=GET&url=https%3A%2F%2Fwww.codewars.com%2Fapi%2Fv1%2Fusers%2Fscorwin') //https://www.codewars.com/api/v1/users/spencercorwin
      ...
      .then(data => {
        this.setState({
          ...
          js: data.ranks.languages.javascript.name,
          jsscore: data.ranks.languages.javascript.score,
          python: data.ranks.languages.python.name,
    ...
    fetch('https://api.stackexchange.com/2.2/users/9157715?order=desc&sort=reputation&site=stackoverflow')
      .then(res => res.json())
      .then(data => {
        this.setState({
  ...
  render() {
    return (
      <div className={this.state.dark ? 'body-container light-body' : 'body-container dark-body'}>
        <div className="body">
